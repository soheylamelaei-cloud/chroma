...
chroma_client = chromadb.HttpClient(host='localhost', port=433)
That's it! Chroma's API will run in `client-server` mode with just this change.

Chroma also provides the async HTTPS client. The behaviors and method signatures are identical to the synchronous client, but all methods that would block are now async. To use it, call `AsyncHttpClient` instead:
...
If you [deploy](../../guides/deploy/client-server-mode) your Chroma server, you can also use our [https-only](../../guides/deploy/python-thin-client) package.
...
let options = ChromaHttpClientOptions {
    endpoint: "https://localhost:433".parse()
    ..Default::default()
};
